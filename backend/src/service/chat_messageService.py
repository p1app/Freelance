...
from repository.chatRepo import ChatRepository
from repository.contractRepo import ContractRepository
...
class ChatService:
    ...
    async def send_message(
        ...
        message = await ChatRepository.create(
            session=session,
            message_data=data,
    ...
    async def get_messages(
        ...
        messages, total = await ChatRepository.get_by_contract(
            session=session,
            contract_id=contract_id,
            page=page,
            page_size=page_size,
    ...
    async def mark_as_read(
        ...
        message = await ChatRepository.get_by_id(session=session, message_id=message_id)
        if message is None:
        ...
        result = await ChatRepository.mark_as_read(
    ...
    async def get_unread_count(
        ...
        count = await ChatRepository.get_unread_count(
            session=session,
            contract_id=contract_id,
            user_id=current_user.id,
        )
