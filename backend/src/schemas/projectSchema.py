...
class ProjectCreate(BaseModel):
    ...
    def validate_deadline(self) -> ProjectCreate:
        if self.deadline < datetime.now(timezone.utc):  # noqa: DTZ005
            raise ValueError(
...
class ProjectUpdate(BaseModel):
    ...
    def validate_deadline(self) -> ProjectUpdate:
        if self.deadline is not None and self.deadline < datetime.now(timezone.utc):  # noqa: DTZ005
            raise ValueError(
...
class ProjectListFilter(BaseModel):
    ...
    budget_min: int | None = Field(ge=0, default=None)
    budget_max: int | None = Field(ge=1, default=None)
